...
author: "Andreas Scheidegger, Sandrine Charles, Roman Ashauer, Nele Schuwirth"
...
- Supporting information for the paper:
  "Three Perspectives on Model Building and Calibration for Ecological Risk Assessment" 
  by Schuwirth, Ashauer, Baity-Jesi, Charles, Galic, Moor, Preziosi, Scheidegger, Schmolke & Strauss
- The dataset used is from Ashauer (2010).
- For simplicity of this didactical example, we use the limit of quantification 
  (LOQ) divided by 2 for below-detection data, which is a common but not entirely 
  correct approach.
- The priors are based on Hendriks et al. (2001).
- The code and input can be received from https://github.com/Eawag-SIAM/3-perspective-example

# Introduction

This example provides an analysis using a one-compartment
toxicokinetic model. The data set used comes from *Gammarus pulex*
...
This is a didactical toy example with the goal, to demonstrate
the technical implementation of all three perspectives. 
It is not a realistic application case for perspectives 1 and 3.
The results are not representative for typical use cases.

# Import Data

We load the data set containing measurements of internal
concentrations in *Gammarus pulex* over time. The data set is
...
observed data. We also sample parameter uncertainty and plot the 90% prediction 
interval. For the prior distribution, we assume lognormal distributions
to ensure that the parameters can not be negative. They are defined by the mean 
and standard deviation of each parameter. Since we do not have prior knowledge 
about their correlation structure, we assume that the priors of the parameters 
are independent from each other. Since in this case the prior knowledge is vague, 
it results in large uncertainty intervals.
...
# Perspective 2: Maximum Likelihood Estimation
We estimate the parameters by maximizing the likelihood of the observed data, 
assuming a) normal- and b) gamma-distributed errors. 
The normal distribution is a typical assumption, but it can lead to negative 
parameter values. The gamma distribution ensures that the parameters are positive.
We then run the model using these MLE parameters and apply bootstrapping to 
propagate uncertainty.

### *a) MLE with Normal-distributed errors
...
simu.mle.P2.n <- bioacc(parameters.P2.n, C_water, tc, tsim)

ggplot(simu.mle.P2.n, aes(time, conc)) +
  geom_line(col = "orange", linewidth = 1.5) +
  xlab("Time (days)") +
  ylab("Internal predicted concentrations") +
  geom_vline(xintercept = 1, linetype="dashed") +
...
### *b) MLE with Gamma-distributed errors
We repeat the above process for gamma-distributed errors to avoid that 
concentrations can become negative.
...
simu.mle.P2.g <- bioacc(parameters.P2.g, C_water, tc, tsim)

ggplot(simu.mle.P2.g, aes(time, conc)) +
  geom_line(col = "orange", linewidth = 1.5) +
  xlab("Time (days)") +
  ylab("Internal predicted concentrations") +
  geom_vline(xintercept = 1, linetype="dashed") +
...
## *Bootstrapping to propagate uncertainty
...
ggplot() +
  ...
  geom_line(data = simu.mle.P2.n, aes(time, conc), col = "orange", linewidth = 1.5) +
  xlab("Time (days)") +
  ylab("Internal concentration") +
  geom_vline(xintercept = 1, linetype="dashed") +
  geom_point(data = df, aes(time, Cinternal)) +
...
ggplot() +
  ...
  geom_line(data = simu.mle.P2.g, aes(time, conc), col = "orange", linewidth = 1.5) +
  xlab("Time (days)") +
  ylab("Internal concentration") +
  geom_vline(xintercept = 1, linetype="dashed") +
  geom_point(data = df, aes(time, Cinternal)) +
...
We incorporate prior knowledge into the model using a Bayesian approach, 
which allows us to sample from the posterior distribution of the parameters 
and estimate the uncertainty in our predictions.
...
We define functions proportional to the posterior distribution, incorporating 
both likelihood and prior information.
...
We sample from the posterior distribution using a Markov chain Monte Carlo (MCMC) 
method. This allows us quantify the uncertainty in the model predictions.
...
# get maximum posterior parameters and model results
map.index <- which(pp.gamma$log.p == max(pp.gamma$log.p))[1]
map.estimate <- pp.gamma$samples[map.index, ]
simu.map.P3.g <- bioacc(map.estimate, C_water, tc, tsim)
...
Finally, we plot the 90% credible intervals for normal and gamma errors, 
incorporating both parameter and total uncertainty.
...
comparing the Bayesian prior and the posterior distributions. Note that, strictly
speaking, the bootstrap distribution from P2 cannot be directly compared to the
...
estimator of the parameter. We show it here for illustration. 
...
P1 <- ggplot(intervals.para.P1) +
  ...
  geom_line(aes(x=time, y=p50), col = "orange", linewidth = 1.5) +
  xlab("Time (days)") +
  ylab("Internal concentration (picomol/g wet weight)") +
  geom_vline(xintercept = 1, linetype="dashed") +
  geom_point(data = df, aes(time, Cinternal)) +
...
P2 <- ggplot(intervals.para.P2.g) +
  ...
  geom_line(data = simu.mle.P2.g, aes(time, conc), col = "red", linewidth = 1.5) +
  geom_line(aes(x=time, y=p50), col = "orange", linewidth = 1.5) +
  xlab("Time (days)") +
  ylab("Internal concentration (picomol/g wet weight)") +
  geom_vline(xintercept = 1, linetype="dashed") +
...
P3 <- ggplot(intervals.para.P3.g) +
  geom_ribbon(aes(x=time, y=p50, ymin=p05, ymax=p95), alpha = 0.2) +
  geom_ribbon(data=intervals.total.P3.g, aes(x=time, y=p50, ymin=p05, ymax=p95), alpha = 0.2) +
  geom_line(data = simu.map.P3.g, aes(time, conc), col = "red", linewidth = 1.5) +
  geom_line(aes(x=time, y=p50), col = "orange", linewidth = 1.5) +
  xlab("Time (days)") +
  ylab("Internal concentration (picomol/g wet weight)") +
  geom_vline(xintercept = 1, linetype="dashed") +
...
Ashauer, R., Caravatti, I., Hintermeister, A. & Escher, B.I. (2010). Bioaccumulation kinetics of organic xenobiotic pollutants in the freshwater invertebrate Gammarus pulex modeled with prediction intervals. Environmental Toxicology and Chemistry, 29, 1625-1636.

Hendriks, A.J., van der Linde, A., Cornelissen, G. & Sijm, D.T.H.M. (2001). The power of size. 1. Rate constants and equilibrium ratios for accumulation of organic substances related to octanol-water partition ratio and species weight. Environmental Toxicology and Chemistry, 20, 1399-1420.