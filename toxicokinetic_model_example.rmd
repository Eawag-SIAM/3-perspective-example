...
- add prior, posterior, and bootstap CI
...
Here, we load the dataset which contains measurements of internal
concentrations in *Gammarus pulex* over time. The dataset is
visualized to understand the time course of internal concentration
Unfortunately we do not know which points belong to the same replica.
...
water and the simulation times. Note, the implementation assumes that
`tsim` is ordered.
...
mle = maxLik(log.ll.normal, start=c(k.u=90, k.e=1, log.sd=log(1)), data=df, LOQ = LOQ, method="NM")
summary(mle)
...
mle.g = maxLik(log.ll.gamma, start=c(k.u=80, k.e=0.8, log.sd=log(1)), data=df, LOQ=LOQ, method="NM")
summary(mle.g)
...
The maximum likelihood estimation computes confidence intervals of the
parameters based on large sample size approximation. Bootstrapping
avoids this approximations and also offers a straight-forward way to
propagate the parameter uncertainty through the model to compute
...
for(i in 1:M){
    ...
    fit <- maxLik::maxNM(log.ll.normal,
                         ...
                         parscale = c(100, 1, 5),
                         constraints = list(ineqA=diag(3), ineqB=c(0,0,5)) #  A %*% theta + B > 0
                         )
